...
class HttpsCertificatePinner(
        ...
        return object : HttpClientPlugin<Unit, HttpsCertificatePinner> {
            ...
            override fun install(plugin: HttpsCertificatePinner, scope: HttpClient) {
                ...
                scope.responsePipeline.intercept(HttpResponsePipeline.After) { response ->
                    val hostname = context.request.attributes.getOrNull(HOSTNAME_KEY)
                    if (hostname != null) {
                        // This is where we would validate the certificate in a real implementation
                        // For now, we'll simulate the validation
                        plugin.validateConnectionSecurity(hostname)
                    }
                    proceed()
                }
            }
    ...
    suspend fun validateCertificate(hostname: String, certificateData: ByteArray): Result<Boolean> {
        return try {
            ...
            cacheMutex.withLock {
                ...
                validationCache[cacheKey] = ValidationResult(isValid, Clock.System.now().toEpochMilliseconds())
            }
     ...
     */
    private suspend fun validateConnectionSecurity(hostname: String) {
        try {
    ...
    data class ValidationResult(
        ...
        fun isNotExpired(): Boolean = (Clock.System.now().toEpochMilliseconds() - timestamp) < CACHE_TTL_SECONDS * 1000
    }
